...
class BSTree:
    ...
    def add_node(self,node):
        if self.root is None:
            ...
            self.addNodeAux(self.root, node)

        return

    def addNodeAux(self,root, node):
        if node.value > root.value:
            if root.right == None:
                root.right = node
                return
            else:
                self.addNodeAux(root.right, node)
        else:
            if root.left == None:
                root.left = node
                return
            else:
                self.addNodeAux(root.left, node)

    def print_post_order(self):
        self.printPostOrderAux(self.root)

    def printPostOrderAux(self,root):
        if root is None:
            return
        if root.left is not None:
            self.printPostOrderAux(root.left)
        print root.value,
        if root.right is not None:
            self.printPostOrderAux(root.right)
...
class DLinkQueue:
    ...
    def set_queue(self, bsTree):
        if bsTree.root is None:
            ...
            return
        self.setQueueAux(bsTree.root)

    def setQueueAux(self, root):
        if root.left is not None:
            self.setQueueAux(root.left)
        self.add_node_to_tail(root)
        if root.right is not None:
            self.setQueueAux(root.right)
